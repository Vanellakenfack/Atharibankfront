// pages/ChefComptableDashboard.tsx
...
const ChefComptableDashboard = () => {
  ...
  // Filter applications based on processing status and amount threshold
  const getFilteredApplications = () => {
    let filtered = applications;

    // Show applications with APPROUVE or REJETE status AND exceed 500,000 FCFA (Chef Comptable threshold)
    filtered = filtered.filter(app =>
      (app.statut === 'APPROUVE' || app.statut === 'REJETE') && parseFloat(app.montant || 0) > 500000
    );

    // Filter by tab value
    switch (tabValue) {
      case 0: // Toutes (applications APPROUVE and REJETE)
        filtered = filtered;
        break;
      case 1: // Approuvées (APPROUVE status)
        filtered = filtered.filter(app => app.statut === 'APPROUVE');
        break;
      case 2: // Rejetées (REJETE status)
  ...
  const loadApplications = async () => {
    try {
      ...
      if (data.status === 'success') {
        // Filter applications for chef comptable (where statut is APPROUVE or REJETE)
        const filteredApps = (data.data || []).filter(app => {
          return (app.statut === 'APPROUVE' || app.statut === 'REJETE') && parseFloat(app.montant || 0) > 500000;
        });
  ...
  const handleGiveOpinion = (application) => {
    setSelectedApplication(application);
    setCurrentOpinion({ opinion: '', commentaire: '', niveau_avis: 'GENERAL' });
  ...
  const handleSubmitOpinion = async () => {
    ...
    try {
      ...
      const requestBody: any = {
        ...
        niveau_avis: currentOpinion.niveau_avis || 'GENERAL',
        role: 'chef_comptable'
      };
      ...
      setCurrentOpinion({ opinion: '', commentaire: '', niveau_avis: 'GENERAL', score_risque: '' });
  ...
  const calculateAmortization = (montant, duree, interet_total, frais_dossier, frais_etude) => {
    const totalAmount = parseFloat(montant || 0) + parseFloat(interet_total || 0) + parseFloat(frais_dossier || 0) + parseFloat(frais_etude || 0);
    const dailyPayment = totalAmount / parseFloat(duree || 1);
    const amortizationData = [];
    let remainingBalance = totalAmount;
    let cumulativePaid = 0;

    for (let i = 1; i <= parseFloat(duree || 1); i++) {
  ...
  const renderAmortizationTable = () => {
    ...
    const data = calculateAmortization(
      ...
      selectedApplication.frais_dossier,
  ...
  const stats = {
    ...
    highValueApplications: applications.filter(app => parseFloat(app.montant || 0) > 500000).length
  };
      ...
      >
        ...
        <Container maxWidth="lg" sx={{ py: 4, flex: 1 }}>
          <Typography variant="h4" gutterBottom sx={{ fontWeight: 'bold', color: '#1976d2', mb: 4 }}>
            ...
            Tableau de Bord Chef Comptable
          </Typography>
                ...
                <CardContent>
                  ...
                  <Typography variant="h4" component="div" sx={{ fontWeight: 'bold' }}>
                    {applications.filter(app => {
                      const comptableAvis = app.avis?.find((avis: any) => avis.role === 'chef_comptable');
                      return comptableAvis && comptableAvis.opinion === 'approuver';
                    }).length}
                  </Typography>
                  <Typography variant="body2">
                    Demandes approuvées
              ...
              <Card sx={{ bgcolor: 'error.light', color: 'error.contrastText' }}>
                <CardContent>
                  ...
                  <Typography variant="h4" component="div" sx={{ fontWeight: 'bold' }}>
                    {applications.filter(app => {
                      const comptableAvis = app.avis?.find((avis: any) => avis.role === 'chef_comptable');
                      return comptableAvis && comptableAvis.opinion === 'rejeter';
                    }).length}
                  </Typography>
                  <Typography variant="body2">
                    Demandes rejetées
          ...
          <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
            <Tabs value={tabValue} onChange={(event, newValue) => setTabValue(newValue)} aria-label="application tabs">
              <Tab label={`Toutes (${applications.length})`} />
              <Tab label={`Approuvées (${applications.filter(app => app.statut === 'APPROUVE').length})`} />
                ...
                <Box sx={{ mt: 2 }}>
                  ...
                  <Grid container spacing={2} sx={{ mb: 3 }}>
                    <Grid item xs={12} sm={6}>
                      ...
                      <Typography variant="body1" fontWeight="medium">{formatCurrency(parseFloat(selectedApplication.montant || 0))}</Typography>
                    </Grid>
                    <Grid item xs={12} sm={6}>
                  ...
                  >
                    <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      <DescriptionIcon />
                      Donner un avis - Application #{selectedApplication?.numero_demande}
                    </DialogTitle>
                    <DialogContent>
                      {selectedApplication && (
                        <OpinionForm
                          ...
                          onCancel={() => setOpinionDialogOpen(false)}
                          loading={givingOpinion}
                        />
                      )}
                    </DialogContent>
                  </Dialog>
                  ...
                  >
                    <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      <PictureAsPdfIcon />
                      Génération du PV - Application #{selectedApplication?.numero_demande}
                    </DialogTitle>
                    <DialogContent>
                      {selectedApplication && (
                        <PVGeneration
                  ...
                  >
                    <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      <ReceiptIcon />
                      Mouvement Comptable - Application #{selectedApplication?.numero_demande}
                    </DialogTitle>
                    <DialogContent>
                      {selectedApplication && (
                        <AccountMovement
...
export default ChefComptableDashboard;
